...
namespace WebApplicationHardcodedEncrypt.Controllers
{
    public class SecureController : Controller
    {
        public ActionResult Index()
        {
            ...
            using (SymmetricAlgorithm algorithm = GetSymmetricAlgorithm(context))
            {
                context.SaveChanges();
        ...
        private SymmetricAlgorithm GetSymmetricAlgorithm(ApplicationDbContext context)
        {
            ...
            byte[] keyBytes = Encoding.ASCII.GetBytes(context.Settings.FirstOrDefault(x => x.Key == "EncryptionKey")?.Value ?? "");
